...
if os[:name] == 'ubuntu'
  ...
  describe file('/etc/cloudflared/config.yml') do
    ...
    its('content') { should match /service: https:\/\/10\.1\.2\.3:443/ }
    its('content') { should match /noTLSVerify: true/ }
    its('content') { should match /noTLSVerify: false/ }
    its('content') { should match /service: ssh:\/\/gateway\.internaldomain\.net:22/ }
  end
